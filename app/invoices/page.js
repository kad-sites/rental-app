...
export default function InvoicesPage() {
  ...
  const handleSendManual = async (inv) => {
    ...
    let payUrl = `https://${cleanBaseUrl}/pay/${inv.id}`;
                    ...
                    <td style={{display: 'flex', gap: '0.5rem', whiteSpace: 'nowrap'}}>
                      ...
                      {inv.status !== 'PAID' ? (
                        <>
                          ...
                          >
                            {sendingId === inv.id ? '...' : 'Twilio'}
                          </button>

                          <button 
                            className="btn btn-outline" 
                            style={{padding: '0.4rem 0.6rem', fontSize: '0.75rem', borderColor: 'var(--text-secondary)', color: 'var(--text-secondary)'}}
